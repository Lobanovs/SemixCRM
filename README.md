...
|---|---|---|
| Главная | Навигация, поиск по разделам, встроенная инструкция, светлая и тёмная темы | Метрики считаются по живым данным разделов; тема сохраняется в `localStorage` |
...
Backend предоставляет 52 прикладные API-операции на 40 маршрутах. При старте он создаёт или обновляет схему SQLite, восстанавливает настройки фриланс-снайпера и запускает его фоновый поток, если снайпер был включён ранее. При остановке backend гасит все процессы проектов, которые запускал.
...
лимит модели. Запрос к AI отправляется только после нажатия «Сгенерировать 3 текста».

Перед генерацией CRM читает реальные отзывы из публичной карточки 2GIS, показывает
найденные подтверждения и позволяет добавить своё наблюдение о бизнесе. Модель готовит
три разных варианта: подробный заход через отзывы и точку роста, более прямое предложение
решения и короткий первый контакт. Тексты можно редактировать перед отправкой. Ссылка на
портфолио `https://semyon-lobanov-portfolio.vercel.app/` добавляется по умолчанию, но
убирается одним переключателем сразу из всех трёх вариантов без повторной генерации.
...
- **Промпт запрещает спам и выдумки.** «Уникальное предложение», «под ключ»,
  «команда профессионалов», давление и неподтверждённые цифры отсекаются инструкцией,
  а результат дополнительно проверяется кодом.
- **Персонализация подтверждается отзывами.** Backend извлекает короткие фрагменты
  реальных отзывов 2GIS, присваивает им идентификаторы и требует от модели сослаться
  только на переданные подтверждения. Если отзывы недоступны, текст честно строится
  по остальным данным карточки и ручному наблюдению.
- **Не обещаем невозможное.** Промпт не предлагает заранее сделанное персональное демо.
  Вместо этого сообщение объясняет найденную точку роста, решение, способ получения
  заявок и предлагает прислать стоимость и сроки без обязательного созвона.
- **Результат кэшируется** по отпечатку входа. Повторное открытие карточки не стоит
  ...
  помечается как устаревший. Кнопка «Переписать 3 варианта» игнорирует кэш.
- **Ничего не отправляется автоматически.** Текст правится прямо в окне, дальше вы
  ...
  в статус «Написал».
- **Профиль исполнителя** — кто пишет, с каким стеком, ценами и первым шагом —
  хранится один раз и подставляется в каждый промпт: `GET/PUT /api/ai/profile`.
...
| AI и настройки | безопасное чтение и сохранение OpenCode Go, удаление ключа, проверка подключения, профиль исполнителя и генерация сообщений |
...
- маршруты, которые запускают процессы и браузеры — `/api/projects/{id}/start` и `/stop`,
  `/api/jobs/parse`, `/api/freelance/sniper/check`, `/api/freelance/sources/{source}/auth` —
  дополнительно требуют заголовок `X-Requested-With: SemixCRM`, который нельзя выставить
...
Сейчас это 224 backend-теста и 58 frontend-тестов.
