...
В карточке клиента есть кнопка «Написать». Она разбирает компанию по данным из базы —
рейтинг, число отзывов, наличие настоящего сайта, каналы связи, очки лида и причины
скоринга — и готовит три варианта первого сообщения с разными заходами: наблюдение
о них, аргумент про деньги и очень короткий вариант. Плюс формулировку боли, объяснение
выгоды в рублях и текст напоминания на случай молчания через три дня.
...
- **Промпт запрещает спам.** «Меня зовут», «уникальное предложение», «под ключ»,
  «команда профессионалов», закрытые вопросы в конце и ссылки в первом сообщении
  отсекаются инструкцией, а на выходе ещё раз проверяются кодом и попадают
  в предупреждения, если модель их всё-таки вставила.
- **Считаем деньги, а не функции.** В промпт передаются ориентиры среднего чека
  по нише — явно помеченные как оценка, не как факты о компании. Выдумывать
  статистику модели запрещено отдельным пунктом.
- **Только реальные данные.** Чего нет в карточке, того нет и в сообщении:
  отсутствующий рейтинг подаётся как «нет данных», а не додумывается.
- **Результат кэшируется** по отпечатку входа. Повторное открытие карточки не стоит
  ...
  сбрасывается сам. Кнопка «Переписать заново» игнорирует кэш.
