...
> **Статус проекта:** готовая к ежедневной локальной работе однопользовательская CRM. Данные хранятся в SQLite, ключ OpenCode Go настраивается из интерфейса, а базу можно резервировать без остановки backend.

## Что умеет проект

| Раздел | Возможности | Состояние данных |
|---|---|---|
| Главная | Центр действий «Сегодня»: задачи, лиды, вакансии, фриланс-заказы и состояние источников | Агрегация по живым данным; быстрые переходы к нужному разделу |
...
| Настройки | OpenCode Go, модель и таймаут, рабочий AI-профиль, тест подключения, резервные копии базы | Рабочий API; ключ и профиль в SQLite, копии в `backend/data/backups` |
...
Backend предоставляет 76 прикладных API-маршрутов. При старте он создаёт или обновляет схему SQLite, восстанавливает настройки фриланс-снайпера и запускает его фоновый поток, если снайпер был включён ранее. При остановке backend гасит все процессы проектов, которые запускал.
...
Скопируйте `.env.example` в `.env` и заполните нужные поля. Backend загружает этот файл автоматически; значения, явно заданные в PowerShell, имеют приоритет.
...
| `SEMIXCRM_BACKUP_DIR` | Альтернативный каталог резервных копий | `backend/data/backups` |
...
Каталог `backend/data/` исключён из Git. В «Настройках» можно создать целостную резервную копию базы без остановки backend, скачать её и хранить отдельно. Локальная история лежит в `backend/data/backups/`.
...
| Резервные копии | `GET/POST /api/backups`, скачивание проверенного SQLite-снимка |
...
| `npm run check` | Запустить все frontend-тесты и production-сборку |
...
Сейчас это 314 backend-тестов и 115 frontend-тестов. Тот же прогон выполняет GitHub Actions при каждом push и pull request.
...
│   ├── backups.py          # целостные SQLite-снимки и их выдача
│   ├── config.py           # безопасная загрузка локального .env
...
├── .env.example
├── .github/workflows/ci.yml # проверка backend, frontend и production-сборки
├── package.json
└── tsconfig.json
```
...
- нет облачного развёртывания и Docker-конфигурации; CI проверяет backend и frontend;
